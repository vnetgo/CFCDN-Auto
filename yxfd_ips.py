...
FILE_PATH = 'yxfd_ips.txt'
FDCS_FILE_PATH = 'CloudflareST/fdcs.txt'
...
def get_ip_data():
    ...
    # 从本地文件获取IP数据
    ip_list2 = []
    if os.path.exists(FDCS_FILE_PATH):
        with open(FDCS_FILE_PATH, 'r') as f:
...
def filter_and_format_ips(ip_list):
    ...
    for ip in ip_list:
        ...
        try:
            ...
            if results['network']['country'] == 'TW':
                taiwan_ips.append(f"{ip}#TW")
...
# 第三步：将格式化后的台湾IP地址写入到yxfd_ips.txt文件
...
# 第五步：更新Cloudflare域名的DNS记录为yxfd_ips.txt文件中的IP地址
...
def main():
    ...
    # 第二步：过滤并格式化台湾IP地址
    taiwan_ips = filter_and_format_ips(cleaned_ip_list)

    # 新步骤：去除重复的IP地址
    unique_taiwan_ips = remove_duplicate_ips(taiwan_ips)

    # 如果没有找到符合条件的新加坡IP，则不执行任何操作
    if not unique_taiwan_ips:
        print("No taiwan IPs found. Keeping existing yxfd_ips.txt file.")
        return

    # 第三步：将格式化后的台湾IP地址写入文件
    write_to_file(unique_taiwan_ips)
